...
#busca el proceso con que menor tiempo de cpu necesita
# /E $1 = procesoActual, $? idProceso mas corto en memoria
function buscaSiguienteProceso {

  #partimos de que el el proceso mas corto es el que se nos pasa por parametro
  local elegido=$1;
  local minimo=${tiemposDeCpu[$1]};

  for (( candidato = 0; candidato < ${#tiemposDeCpu[@]}; candidato++ )); do

    #$[ {procesosEnMemoria[$candidato]} -eq 1 ] &&

     if [ ${tiemposDeLlegada[$candidato]} -le ${clock} ] \
  ...
  if [ $isManual = "S" ] 2>/dev/null || [ $isManual = "s" ] 2>/dev/null;then
  	...
  	while [ $t -eq 0 ];do
        ...
        error=0
            ...
            for j2 in ${nombresProcesos[$(expr $i - 1)]}; do
              let x2++
            done
            ...
            if [ $x2 -ne 1 ];then #Si es distinto significa que el nombre tiene espacios
  			...
  			while [ $j -eq 0 ];do
  				...
  				if [ "${memoriaNecesaria[$(expr $i-1)]}" -le $totalMemoria -a "${memoriaNecesaria[$(expr $i-1)]}" -gt 0 ] 2> /dev/null ;then
  					j=1
  				else
  					imprimeError "Dato incorrecto"
  				fi
...
function recolectaBasura {
    ...
    for (( i = 0; i < ${#nombresProcesos[@]}; i++ )); do
      ...
      if [ ${procesosEnMemoria[$i]} -eq 1 ] && [ ${tiemposDeCpu[$i]} -eq 0 ]; then
        ...
        echo "se ventila" ${nombresProcesos[$i]} ${memoriaNecesariaI[$i]} ${memoriaNecesariaF[$i]}
...
function asignaMemoria {
  ...
  for (( i = 0; i < ${#nombresProcesos[@]}; i++ )); do
    ...
    if [ ${procesosEnMemoria[$i]} -eq 0 ] && [ ${tiemposDeCpu[$i]} -gt 0 ] && [ ${tiemposDeLlegada[$i]} -lt $reloj ]; then
      ...
      if [[ ${memoriaNecesaria[$i]} -le $(expr $totalMemoria - $totalMemoriaOcupada) ]]; then
        ...
        for (( h = $inicio; h < $totalMemoria; h++ )); do
          ...
          if [ ${memoria[$h]} != $Li ]; then
              ...
              if [[ $contador -ge ${memoriaNecesaria[$i]} ]]; then
                echo "alojando" ${nombresProcesos[$i]} $contador ${memoriaNecesaria[$i]}
        ...
        echo "Houston! tenemos un problema, no hay memoria para "${nombresProcesos[$i]}" se intentara alojar después :("
      fi
...
while [[ $finDeLaPlanificacion -eq 0 ]]; do
  ...
  for (( candidato = 0; candidato < ${#tiemposDeCpu[@]}; candidato++ )); do
     ...
     if [ ${tiemposDeLlegada[$candidato]} -le ${reloj} ] && [ ${tiemposDeCpu[$candidato]} -lt ${minimo} ] && [ ${tiemposDeCpu[$candidato]} -ne 0 ]; then
  ...
  echo "tiempo de "${nombresProcesos[$procesoActual]} "= " ${tiemposDeCpu[$procesoActual]}
